...
export function MutationDropdown({ imageSrc, listPosition = 'right' }) {
  const { mutations, selectedMutation, switchMutation, isLoading } = useMutableWeb()
    ...
    >
      ...
      {selectedMutation ? (
        <ActiveMutation onClick={handleDropdownToggle}>
          <MutationTitle
            title={selectedMutation.metadata?.name || parseMutationId(selectedMutation.id).localId}
          >
            {selectedMutation.metadata?.name || parseMutationId(selectedMutation.id).localId}
          </MutationTitle>
              ...
              return (
                ...
                >
                  ...
                  <MutationInfo onClick={() => handleMutationClick(mutation)}>
                    <MutationItemTitle>
                      <span className="titleMutation">{mutation.metadata?.name || localId}</span>
                    </MutationItemTitle>
