...
const Agent: FC<TAgentsProps> = ({ agent, onDisconnect }) => {
  ...
  return (
    <div className="flex w-full items-center justify-between gap-3.5 rounded-[10px] bg-(--color-light-white-bg) p-2.5">
      ...
      <div
        className={`flex w-15 items-center justify-center rounded-[10px] bg-(--color-light-white-bg) py-3 text-xs/[100%] font-normal text-(${agent.status === 'active' ? '--color-primary' : '--color-gray-text'}) capitalize`}
      >
        {agent.status}
      </div>
      <button
