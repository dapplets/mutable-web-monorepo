...
export const DropdownMenu: React.FC<{
  ...
  onFavoriteMutationClick: (mutationId: string) => void
}> = ({ onVisibilityChange, onMutateButtonClick, onFavoriteMutationClick }) => {
  const { selectedMutation, favoriteMutationId, switchMutation } = useMutableWeb()
  const { mutations, isLoading } = useMutations()
  ...
  if (isLoading) {
    return (
      <MutationsList>
        <MutationsListWrapper>Loading...</MutationsListWrapper>
      </MutationsList>
    )
  }
        ...
        {Object.keys(recentlyUsedMutations).length > 0 ? (
          ...
          >
            {Object.values(recentlyUsedMutations).map((muts) => {
              ...
              const recentlyUsedSource = muts[0].settings.preferredSource // ToDo: why first mutation ?
              const mut =
                muts.find((mut) => mut.source === (recentlyUsedSource ?? EntitySourceType.Local)) ??
                muts[0]
              return (
                <InputBlock key={mut.id} isActive={mut.id === selectedMutation?.id}>
                  ...
                  {mut.id === favoriteMutationId ? (
                    <InputIconWrapper onClick={() => onFavoriteMutationClick(mut.id)}>
                      <StarMutationList />
                    </InputIconWrapper>
          ...
          <ModalDelete
            onAction={async () => {
              ...
              if (mutationIdToDelete === favoriteMutationId)
                onFavoriteMutationClick(mutationIdToDelete)
