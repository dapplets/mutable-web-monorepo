...
export default class extends ethers.Signer implements EthereumWallet {
  ...
  async getAddress(): Promise<string> {
    ...
    if (!this._wallet) return '' // ToDo: check it
  ...
  async connectWallet(): Promise<void> {
    await this._initWallet()
    let privateKey =
      ((await browser.storage.local.get('dapplets_privateKey')).dapplets_privateKey as string) ?? ''
    if (!privateKey) {
  ...
  async getMeta() {
    return {
      ...
      icon: '', // walletIcons['dapplets'], // ToDo: add icon
    }
  }

  async getLastUsage() {
    return (
      ((await browser.storage.local.get('dapplets_lastUsage')).dapplets_lastUsage as string) ?? ''
    )
  }

  private _initWallet = async () => {
    if (!this._wallet) {
      const privateKey = (await browser.storage.local.get('dapplets_privateKey')) as {
        [name: string]: string
      }
